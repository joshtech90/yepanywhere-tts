...
| Codex | The pinned `0.147.0` app-server source has explicit `thread/name/set` but no automatic title, away-recap, progress-summary, or prompt-suggestion generator. The legacy rollout/database `title` preview is not the optional `thread.name`. These features are believed absent. |
