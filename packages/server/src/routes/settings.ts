  ...
  app.put("/", (c) =>
    serializeSettingsUpdate(async () => {
      ...
      if ("autoSessionTitle" in body) {
        if (
          body.autoSessionTitle === undefined ||
          body.autoSessionTitle === null
        ) {
          ...
          // Merge onto the stored value so a partial patch (just `enabled`,
          // say) keeps the other fields instead of resetting them to defaults.
